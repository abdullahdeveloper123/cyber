...
function Navbar() {
    ...
    // Trigger count update when user changes (login/logout)
    useEffect(() => {
        const updateCounts = () => {
            const cart = JSON.parse(localStorage.getItem("cart") || "[]");
            const totalCartItems = cart.reduce((sum, item) => sum + (item.quantity || 1), 0);
    ...
    return (
        <nav className="navbar navbar-expand-lg navbar-custom">
            <div className="container">
                <Link className="navbar-brand brand-logo" to="/"><img src={Logo} alt="" /></Link>
                ...
                <div className="collapse navbar-collapse" id="navbarSupportedContent">
                    ...
                    <ul className="navbar-nav mx-auto mb-2 mb-lg-0 py-3">
                        ...
                        <li className="nav-item">
                            <Link className="nav-link nav-link-custom" to="/blog">Blog</Link>
                        </li>
                    </ul>
                    ...
                    <ul className="navbar-nav mb-2 mb-lg-0 nav-icons">
                        <li className="nav-item">
                            <Link className="nav-link nav-icon-with-badge" to="/wishlist" title="Wishlist">
                                <BsHeart />
                                {wishlistCount > 0 && <span className="nav-badge">{wishlistCount}</span>}
                            </Link>
                        </li>
                        <li className="nav-item">
                            <Link className="nav-link nav-icon-with-badge" to="/cart" title="Cart">
                                <BsCart />
                        ...
                        <li className="nav-item">
                            {user ? (
                                    ...
                                    {isDropdownOpen && (
                                        <>
                                            ...
                                            <div className="user-dropdown-menu p-3">
                                                ...
                                                <Link to="/profile" className="dropdown-item py-2" onClick={closeDropdown}>
                                                    My Profile
                                                </Link>
                                                <button className="dropdown-item " onClick={closeDropdown}>
                                                    Switch Account
                                                </button>
                                                <button className="dropdown-item logout-item py-2 text-danger" onClick={handleLogout}>
                                                    Logout
                                                </button>
                                            </div>
                                        </>
                                ...
                                <Link className="nav-link" to="/login" title="Login"><BsPerson /></Link>
                            )}
                        </li>
                    </ul>
                </div>
